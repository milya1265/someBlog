...
func (r *repository) InsertNewComment(com *Comment) (int, error) {
	log.Println("INFO --> insert comment in database")

	query := "INSERT INTO post_comments (id_post, author, time, body) VALUES ($1, $2, $3, $4) RETURNING id;"

	row := r.DataBase.QueryRow(query, com.IdPost, com.Author, com.Time, com.Body)

	var idCom int

	if err := row.Scan(idCom); err != nil {
		return 0, err
	}
...
func (r *repository) ChangeBody(idCom int, authorId int, newBody string) error {
	...
	if cnt, err := res.RowsAffected(); cnt == 0 {
		...
		err := errors.New("you can't change this comment")
		return err
	}
